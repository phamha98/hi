...
def confirm2FA(driver):
    try:
        ...
        # Tìm tất cả các radio button và nhấp vào phần tử đầu tiên nếu có
        radio_buttons = driver.find_elements(By.CSS_SELECTOR, 
            "section > section.x > div:nth-child(2) > div > div.y.ba > label > input[type=radio]")
        if radio_buttons:
        ...
        # Tìm tất cả các nút submit và nhấp vào phần tử đầu tiên nếu có
...
def loginBy2FA(driver, username, password, code):
    ...
    sleep(3)
    userNameElement = driver.find_element(By.NAME, 'email')
    ...
    faCodeElement = driver.find_element(By.NAME, "approvals_code")
    faCodeElement.send_keys(code)
    time.sleep(2)
# Thay thế bằng selector chính xác
    btn2fa = driver.find_element(By.CSS_SELECTOR, "#checkpointSubmitButton-actual-button")

    btn2fa.click()
    confirm2FA(driver)
    btn2fa = driver.find_element(By.NAME, "checkpointSubmitButton-actual-button")
    if (len(btn2fa) > 0):
        btn2fa.click()
        btn2faContinue = driver.find_element(By.NAME, "checkpointSubmitButton-actual-button")
...
# print(isLogin)
userName = '100033776505359'
passWord = 'NavigationContainer'
twoFa= '854781'
